...
namespace ImGui
{
    ImVec2 calc_some_delta(const ImVec2& P, const ImVec2& Max, const ImVec2& Min) {
        ImVec2 R(P.x * (Max.x - Min.x) + Min.x, P.y * (Max.y - Min.y) + Min.y);
        return R;
    }
    ...
    int Bezier(const char* label, float P[5]) {
        ...
        // header and spacing
        int changed = SliderFloat4(label, P, 0, 1, "%.3f", /*1.0f*/0);
        ...
        Canvas.x += Window->DC.CursorPos.x;
        Canvas.y += Window->DC.CursorPos.y;

        ImRect bb(Window->DC.CursorPos, Canvas);
        ItemSize(bb);
        if (!ItemAdd(bb, NULL))
            return changed;
        ...
        hovered |= 0 != ItemHoverable(ImRect(bb.Min, I), id, ImGuiItemFlags_None);
        ...
        // control points: 2 lines and 2 circles
        {
            ...
            for (int i = 0; i < 2; ++i) {
                pos[i] = calc_some_delta(ImVec2(P[i * 2 + 0], 1 - P[i * 2 + 1]), bb.Max, bb.Min);
            ...
            if (distance[selected] < (4 * GRAB_RADIUS * 4 * GRAB_RADIUS))
            {
                ...
                if (/*hovered &&*/ (IsMouseClicked(0) || IsMouseDragging(0))) {
                    ...
                    static bool area_constrained = AREA_CONSTRAINED;
                    if (area_constrained) {
                        px = (px < 0 ? 0 : (px > 1 ? 1 : px));
                        py = (py < 0 ? 0 : (py > 1 ? 1 : py));
                    }
        ...
        for (int i = 0; i < 3; ++i) {
            ...
            ImVec2 p0 = calc_some_delta(ImVec2(evalx, 1 - 0), bb.Max, bb.Min);
            ImVec2 p1 = calc_some_delta(ImVec2(0, 1 - evaly), bb.Max, bb.Min);
            ImVec2 p2 = calc_some_delta(ImVec2(evalx, 1 - evaly), bb.Max, bb.Min);
        ...
        ImVec2 p1 = calc_some_delta(ImVec2(P[0], 1 - P[1]), bb.Max, bb.Min);
        ImVec2 p2 = calc_some_delta(ImVec2(P[2], 1 - P[3]), bb.Max, bb.Min);
